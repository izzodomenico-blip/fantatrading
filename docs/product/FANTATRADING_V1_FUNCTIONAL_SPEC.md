...
| Rendimento | Punti in classifica | ROI % rispetto al capitale investito |
...
**Crediti:** l'unità di misura del sistema. Ogni partecipante inizia con un budget definito in crediti, acquista giocatori, e chiude la stagione con un valore netto che determina il ROI.

**ROI (Return on Investment):** misura percentuale del rendimento finale. Un ROI positivo significa che il portafoglio vale più di quanto investito; un ROI negativo significa una perdita. La soglia di sopravvivenza è 0%, la soglia premio è 7%.
...
- vedere la classifica generale e la classifica soglia premi;
- leggere il regolamento.

### 2.2 Admin / Organizzatore

Gestisce il ciclo di vita della stagione: carica i dati, lancia i calcoli, supervisiona i risultati, gestisce i premi. Non è un partecipante attivo ma ha accesso completo ai dati.
...
2. Seleziona la stagione e confermazione l'iscrizione.
3. Il sistema crea il portafoglio iniziale del partecipante con il budget definito dalla stagione.
4. Il partecipante può iscriversi solo prima della data di chiusura iscrizioni.

**Vincoli:** iscrizione possibile solo se la stagione è in stato "iscrizioni aperte". Non è possibile iscriversi durante una stagione già in corso (da definire per V1).
...
4. Ogni acquisto scala il budget disponibile del partecipante: `costo = valore giocatore × 1.02`.
5. La rosa non è valida fino al raggiungimento della composizione completa.

**Vincoli:** il budget residuo deve essere sufficiente a completare la rosa. Il sistema deve avvisare se il budget non è sufficiente per completare la composizione obbligatoria.
...
4. Il sistema scalda il costo dal budget del partecipante.
5. Il giocatore entra nella rosa con il valore di acquisto tracciato.

**Vincoli:** non è possibile acquistare un giocatore già in rosa. Il budget residuo deve essere sufficiente a coprire il costo inclusa la commissione. La composizione per ruolo non deve superare i limiti massimi.
...
4. Il sistema aggiunge l'incasso netto al budget disponibile.
...
- budget disponibile;
- valore totale del portafoglio (somma valori correnti);
- commissioni cumulative pagate;
- ROI attuale rispetto al capitale investito iniziale.
...
- commissione pagata;
- budget residuo dopo l'operazione.

### 3.10 Classifiche

Il partecipante accede a:
- **classifica generale:** tutti i partecipanti ordinati per ROI decrescente;
- **classifica soglia premi:** solo i partecipanti con ROI ≥ 7%, ordinati per ROI decrescente.

### 3.11 Premi

Il partecipante vede la struttura premi definita dall'admin:
- soglia per accedere ai premi (ROI ≥ 7%);
- posizione in classifica necessaria per ogni fascia premio;
- montepremi disponibile.

I premi vengono assegnati al termine della stagione dall'admin.
...
2. Crea una nuova stagione con: nome, stagione calcistica di riferimento, data inizio/fine, numero giornate, budget iniziale partecipanti.
3. Configura i parametri regolamentari: commissione acquisto, commissione vendita, platform fee, soglia sopravvivenza, soglia premio, policy SV.
...
2. Il sistema calcola il montepremi disponibile in base alle commissioni generate e alla platform fee.
...
In fondo alla pagina: riepilogo budget disponibile, valore totale portafoglio, commissioni pagate.
...
- valore al momento dell'operazione;
- commissione;
- saldo budget dopo l'operazione.
...
Tabella filtrata: solo partecipanti con ROI ≥ 7%. Mostra la struttura premi e la posizione del partecipante rispetto alle fasce premiate.
...
- calcolo automatico montepremi disponibile.

Al termine della stagione: assegnazione premi ai vincitori, generazione lista finale con importi.
...
| Platform fee | 10% delle commissioni generate |

La platform fee non viene trattenuta dal budget del partecipante: riguarda la quota di commissioni che va alla piattaforma.
...
- `ROI = (valore finale netto − capitale investito) / capitale investito × 100`
- Soglia sopravvivenza: ROI ≥ 0%
- Soglia premio: ROI ≥ 7%
- La classifica finale è ordinata per ROI decrescente.

### 7.9 Montepremi e platform fee

- Il montepremi è finanziato in base alle commissioni generate e alle regole economiche della stagione.
- La platform fee del 10% riguarda esclusivamente le commissioni, non il montepremi totale.
- La struttura premi può essere definita dall'admin per ogni stagione.
...
```
valoreTotalePortafoglio = Σ valoreCorrente(i) per tutti i giocatori in rosa + budgetResiduo
ROI = (valoreTotalePortafoglio − capitaleInvestitoIniziale) / capitaleInvestitoIniziale × 100
```
...
### 9.7 Ricavo piattaforma

```
ricavoPiattaforma = Σ commissioni × platformFeeRate
```

### 9.8 Montepremi disponibile

Il montepremi dipende dalla struttura economica definita dall'admin per ogni stagione. In V1, la piattaforma riceve il 10% delle commissioni totali. Il montepremi ai premi può provenire da quote iscrizione o da accordi separati.
...
- **Pagamenti reali automatici:** la gestione del montepremi reale con pagamenti digitali automatici è fuori scope. I premi vengono gestiti manualmente dall'admin.
...
- iscrizione alla stagione;
- creazione rosa con composizione obbligatoria;
...
### MVP 4 — Classifiche e premi

Obiettivo: la stagione ha un risultato finale con premi.

Contenuto:
- classifica soglia premi (ROI ≥ 7%);
- gestione struttura premi da admin;
- calcolo montepremi e platform fee;
- assegnazione premi a fine stagione;
